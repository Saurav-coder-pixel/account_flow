...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 3) {
      await db.execute(
          'ALTER TABLE persons ADD COLUMN is_cashbook INTEGER NOT NULL DEFAULT 0');
    }
  }
  ...
  Future<List<Person>> getAllPersons() async {
    ...
    final List<Map<String, dynamic>> maps =
        await db.query('persons', orderBy: 'name ASC');
  ...
  Future<List<app_transaction.Transaction>> getTransactionsByPersonId(
      int personId) async {
    final db = await database;
    final List<Map<String, dynamic>> maps = await db.query(
      'transactions',
  ...
  Future<int> deleteTransactionsBySplitId(String splitId) async {
    final db = await database;
    return await db
        .delete('transactions', where: 'split_id = ?', whereArgs: [splitId]);
  }
